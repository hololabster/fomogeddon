...
export const triggerTradeEffect = (type) => {
  ...
  export const triggerPriceAlert = (type, percentChange) => {
    ...
    if (type === 'up') {
      alertElement.innerHTML = `<span>▲</span> ${Math.abs(percentChange).toFixed(2)}%`;
    } else {
      alertElement.innerHTML = `<span>▼</span> ${Math.abs(percentChange).toFixed(2)}%`;
    }
  ...
  export const triggerChartUpdate = (isUpdating) => {
    ...
    if (isUpdating) {
      ...
      loadingIndicator.innerHTML = '<div class="chart-loading-spinner"></div><div class="chart-loading-text">Updating Data...</div>';
