...
export class SimulatorEngine {
  constructor(initialPrice, scenario) {
    ...
    // Extend the event trigger range significantly (50-150 steps)
  ...
  addInitialDataPoints() {
    ...
    // Add 20 historical points with slight variations to create a nice starting chart
    for (let i = 19; i > 0; i--) {
  ...
  step(onChainData = null) {
    ...
    if (onChainData && onChainData.length > 1) {
      ...
      baseChange =
        Math.random() * dynamicVolatility * 2 -
        dynamicVolatility +
        this.trendDirection * this.trendStrength * dynamicVolatility;
    }
    ...
    if (this.eventTriggered && this.eventProgress < this.eventTotalSteps) {
      ...
      const eventRandomness =
        (Math.random() * 0.5 - 0.25) * this.eventImpactPerStep;
