...
export const fetchBitcoinData = async (
  ...
  try {
    ...
    const response = await fetch(
      `${API_OPTIONS.BINANCE}?symbol=BTCUSDT&interval=${interval}&startTime=${startTime}&endTime=${endTime}&limit=2000`
    );
    ...
    try {
      ...
      const response = await fetch(
        `${API_OPTIONS.COINCAP}?interval=${interval}&start=${
      ...
      console.warn(
        "Error fetching from CoinCap, using simulation data:",
        coincapError
      );
...
const generateRealisticBitcoinData = (days = 1) => {
  ...
  for (let i = 0; i < points; i++) {
    ...
    const spike =
      Math.random() > 0.98 ? (Math.random() > 0.5 ? 0.03 : -0.03) : 0;
